...
const getAllNames = arr => {
  return arr.reduce((acc, user) => user.name ? acc.concat(user.name) : acc, []);
};
...
const getTotalBalance = arr => {
  return arr.reduce((acc, x) => acc + x.balance, 0);
};
...
const getAllUniqueSkills = arr => {
  const getAllSkills = arr.reduce((acc, x) => acc.concat(x.skills), []);
  const getUniqueSkills = getAllSkills.filter(function (item, pos) {
    return getAllSkills.indexOf(item) == pos;
  });
  const sortUniqueSkills = getUniqueSkills.sort();
  return sortUniqueSkills;
};
